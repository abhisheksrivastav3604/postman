...
class Home extends Component {
    ...
    async link(url, obj) {
        ...
        let response1 = await http.get(`/history`)
        console.log(response1.data)
        this.setState({ histories: response1.data })
        console.log(status)
        if (data.status >= "400") {
            let s1 = { ...this.state }
            s1.form.response = "Not Found";
            s1.status = data.status
            s1.statusText = "Not Found"
            s1.time = JSON.stringify(data).length
            this.setState(s1)
        }
        else {
            let s1 = { ...this.state }
            s1.form.response = data;
            s1.status = status
    ...
    render() {
        ...
        let { urldata, hide, index, status, statusText, time, history, histories, header,header1 } = this.state;
            ...
            <div className="">
                <div className="row">
                            ...
                            {history == 0 ? "" : <div className="col-8">
                                ...
                                {histories1.map(ele => (
                                    <div className="row overflow-hidden">
                                        <div className="col-2 text-success" style={{ "font-size": "13px" }}>{(ele.method).toUpperCase()}</div>
                                        <div className="col-10" style={{ "font-size": "13px" }}>{ele.fetchurl}</div>
                                    </div>
                                ))}
                            </div>}
                        </div>
                    ...
                    <div className={history == 0 ? "col-11 border " : "col-9 border "}>
                        ...
                        <div className="">
                            <hr />
                            {response == "" || response.code == "ERR_INVALID_URL" || response.code == "ENOTFOUND" ? <div className="col-1 ms-3">Response</div> :
                                <div className="">
                                    <div className="row">
                                        <div className="col-7">
                                            <div className="row d-inline">
                                        ...
                                        <div className="col-5">
                                            <div className="row d-inline">
                                                ...
                                                <div className="d-inline ms-2" style={{ "font-size": "13px" }} >Status : <div className="text-success d-inline">{status} {statusText}</div></div>
                                                <div className="d-inline ms-2" style={{ "font-size": "13px" }} >Time : <div className="text-success d-inline">{time} ms</div></div>
                        ...
                        {/* {hide == true ?
                            <div>{JSON.stringify(urldata)}</div> : ""} */}
                    </div>
                </div>
            </div>
        )
